...
// `rp upgrade` is self-update: the binary replaces itself with the latest GitHub release for the running
// OS/arch, so non-Homebrew installs get the same one-command update as `brew upgrade rp`. When rp was
// installed via Homebrew it refuses and points at `brew update && brew upgrade rp`, so it never fights
// brew's bookkeeping. This is the ONLY command that reaches outside the replypen API.
const (
	ghRepo      = "rootcause-org/replypen-cli"
...
func newUpgradeCmd(e *env, version string) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "upgrade",
		Short: "Update rp to the latest release (self-update)",
		Long: "Update rp to the latest GitHub release for this OS/arch.\n\n" +
			"On Linux/WSL/Windows this replaces the running binary in place. On a Homebrew install it\n" +
			"defers to `brew upgrade rp` instead, so it doesn't fight Homebrew's bookkeeping.",
		Args: cobra.NoArgs,
...
func runUpgrade(e *env, current string, checkOnly bool) error {
	...
	if compareVersions(current, latest) >= 0 {
		fmt.Fprintf(e.out, "rp is already up to date (%s)\n", normVersion(current))
		return nil
	}

	if checkOnly {
		fmt.Fprintf(e.out, "a newer rp is available: %s → %s\n  run: rp upgrade\n", normVersion(current), normVersion(latest))
		return nil
	}

	exe, err := os.Executable()
	if err != nil {
		return fmt.Errorf("cannot locate the running rp binary: %w", err)
	}
	...
	if isHomebrewManaged(exe) {
		fmt.Fprintf(e.out, "a newer rp is available: %s → %s\n"+
			"  this rp was installed with Homebrew — upgrade with: brew update && brew upgrade rp\n", normVersion(current), normVersion(latest))
		return nil
	}

	fmt.Fprintf(e.err, "==> downloading rp %s for %s/%s\n", normVersion(latest), runtime.GOOS, runtime.GOARCH)
	...
	fmt.Fprintf(e.out, "upgraded rp → %s (%s)\n", normVersion(latest), exe)
	return nil
}
...
func httpGet(ctx context.Context, url string) (*http.Response, error) {
	...
	req.Header.Set("User-Agent", "replypen-cli")
...
// checksums.txt, and returns the extracted rp (or rp.exe) bytes.
func fetchReleaseBinary(ctx context.Context, tag, goos, goarch string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("extracting rp from %s: %w", asset, err)
	}
	return bin, nil
}
...
// assetName is the GoReleaser archive name: rp_<ver>_<os>_<arch>.tar.gz (zip on Windows).
func assetName(version, goos, goarch string) string {
	ext := "tar.gz"
	if goos == "windows" {
		ext = "zip"
	}
	return fmt.Sprintf("rp_%s_%s_%s.%s", version, goos, goarch, ext)
}
...
// compareVersions compares dotted numeric versions (leading "v" optional). Returns -1 if a<b, 0 if equal,
// 1 if a>b. Non-numeric/malformed parts make it conservative: unequal strings → "a is older" (-1) so a dev
// build always sees an upgrade rather than wrongly skipping.
...
// replaceBinary writes data over the binary at path atomically: a temp file in the SAME directory (so the
// rename is atomic, not a cross-device copy), then rename into place. On Windows a running .exe can't be
// overwritten, so the current one is moved aside first.
func replaceBinary(path string, data []byte) error {
	dir := filepath.Dir(path)
	tmp, err := os.CreateTemp(dir, ".rp-upgrade-*")
	if err != nil {
		return fmt.Errorf("cannot write to %s (need write permission there; try sudo): %w", dir, err)
	}
	tmpName := tmp.Name()
