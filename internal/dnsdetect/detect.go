...
// uses — and whether ReplyPen can onboard it.
//
// ReplyPen has exactly two provider adapters: "google" (Gmail / Google Workspace)
// and "microsoft" (Microsoft 365 / Graph). Everything else — self-hosted IMAP/SMTP,
// cPanel/Plesk hosting, regional ISPs — is NOT supported. The call is made from
// public DNS so we know before a sales call whether a prospect is onboardable.
//
...
// Result mirrors the Python detect() output dict verbatim (JSON field names).
// replypenProvider is a pointer so it serializes as null when unsupported.
type Result struct {
	Input            string   `json:"input"`
	Domain           string   `json:"domain"`
	Provider         string   `json:"provider"`
	ReplypenProvider *string  `json:"replypen_provider"`
