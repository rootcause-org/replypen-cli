...
func TestDetect(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.supported {
				if r.ReplypenProvider == nil || *r.ReplypenProvider != tc.provider {
					t.Errorf("replypen_provider = %v, want %q", r.ReplypenProvider, tc.provider)
				}
			} else if r.ReplypenProvider != nil {
				t.Errorf("replypen_provider = %v, want nil", *r.ReplypenProvider)
			}
...
// JSON-tagged slices stay non-nil so the wire shape matches the Python dict.
