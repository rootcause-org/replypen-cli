#!/usr/bin/env bash
#
# release.sh — cut a replypen-cli release, end to end, the same way every time.
#
...
#
# RUNBOOK (the whole release is just): tag `vX.Y.Z` → push → the release workflow runs GoReleaser,
# which builds the cross-platform binaries, publishes the GitHub Release, and auto-commits
# Casks/rp.rb to rootcause-org/homebrew-replypen so `brew install rootcause-org/replypen/rp` upgrades.
# This script automates that tag+push and then waits for the assets + warms the proxy.
#
# USAGE:
...
MODULE="github.com/rootcause-org/replypen-cli"
MAIN_BRANCH="main"
...
step "Tag + push"
git tag -a "$VERSION" -m "replypen-cli $VERSION"
...
Consumers can now pull it:
  brew install rootcause-org/replypen/rp     # macOS (or 'brew upgrade rp' if already tapped)
  go get $MODULE@$VERSION          # pin (resolves immediately)
  go install $MODULE/cmd/rp@$VERSION
